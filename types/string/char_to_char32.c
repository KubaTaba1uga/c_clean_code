...
int main(void) {
  ...
  while ((convertion_result =
              mbrtoc32(p_output, p_input, end - p_input, &state))) {
    if (convertion_result == (size_t)-1) { // invalid input
      ...
      goto EXIT_FAILURE;
      break;
    } else if (convertion_result == (size_t)-3) // UTF-32 high surrogate
      p_output += 1;
    else {
  ...
  printf("%zu UTF-32 code units: [ ", output_size);
